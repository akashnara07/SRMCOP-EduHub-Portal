...
// Load JSON data
const sem5Courses = JSON.parse(fs.readFileSync('./src/data/syllabus_sem5.json', 'utf8'));
const sem6Courses = JSON.parse(fs.readFileSync('./src/data/syllabus_sem6.json', 'utf8'));

const allCourses = [...sem5Courses, ...sem6Courses];

async function seedSyllabus56() {
  ...
  for (const course of allCourses) {
    ...
    const semRoman = semester === 5 ? 'Semester V' : 'Semester VI';
  ...
  console.log('Semester V and VI syllabus seeding successfully completed!');
}

seedSyllabus56().catch((error) => {
